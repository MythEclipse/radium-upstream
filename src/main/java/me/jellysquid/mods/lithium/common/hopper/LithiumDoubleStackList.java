 ...
 * The objects should be immutable and their state should be limited to the
 * first and second inventory.
 * Other state must be managed carefully, as at any time objects of this class
...
public class LithiumDoubleStackList extends LithiumStackList {
    ...
    public LithiumDoubleStackList(LithiumDoubleInventory doubleInventory, LithiumStackList first,
            LithiumStackList second, int maxCountPerStack) {
    ...
    public static LithiumDoubleStackList getOrCreate(LithiumDoubleInventory doubleInventory, LithiumStackList first,
            LithiumStackList second, int maxCountPerStack) {
        LithiumDoubleStackList parentStackList = first.parent;
        if (parentStackList == null || parentStackList != second.parent || parentStackList.first != first
                || parentStackList.second != second) {
    ...
    public ItemStack remove(int index) {
        throw new UnsupportedOperationException(
                "Call remove(int value, ItemStack element) on the inventory half only!");
    }
    ...
    public int getSignalStrength(Inventory inventory) {
        // signal strength override state has to be stored in the halves, because this
        // object may be replaced with a copy at any time
        boolean signalStrengthOverride = this.first.hasSignalStrengthOverride()
                || this.second.hasSignalStrengthOverride();
    ...
    /**
     * @param masterStackList the stacklist of the inventory that comparators read
     *                        from (double inventory for double chests)
