...
/**
 * ChunkAwareBlockCollisionSweeper iterates over blocks in one chunk section at
 * a time. Together with the chunk
 * section keeping track of the amount of oversized blocks inside the number of
 * iterations can often be reduced.
 */
public class ChunkAwareBlockCollisionSweeper extends AbstractIterator<VoxelShape> {
    ...
    // variables prefixed with c refer to the iteration of the currently cached
    // chunk section
    ...
    public ChunkAwareBlockCollisionSweeper(World world, Entity entity, Box box) {
        if (!LithiumEntityCollisions.isBoxFinite(box)) {
            ...
            this.context = ShapeContext.absent();
            this.world = world;
            this.minX = this.maxX = this.minY = this.maxY = this.minZ = this.maxZ = 0;
            this.cTotalSize = 0;
            this.cIterated = 1; // Mark as done
            return;
        }
        this.box = box;
        ...
        this.minY = MathHelper.clamp(MathHelper.floor(box.minY - EPSILON), Pos.BlockCoord.getMinY(this.world),
                Pos.BlockCoord.getMaxYInclusive(this.world));
        this.maxY = MathHelper.clamp(MathHelper.floor(box.maxY + EPSILON), Pos.BlockCoord.getMinY(this.world),
                Pos.BlockCoord.getMaxYInclusive(this.world));
    ...
    private boolean nextSection() {
        do {
            do {
                // find the coordinates of the next section inside the area expanded by 1 block
                // on all sides
                // note: this.minX, maxX etc are not expanded, so there are lots of +1 and -1
                // around.
                if (this.cachedChunk != null &&
                    ...
                    // Casting to Chunk is not checked, together with other mods this could cause a
                    // ClassCastException
            ...
            int cEndY = Math.min(this.maxY + sizeExtension,
                    Pos.BlockCoord.getMaxYInSectionIndex(this.world, this.chunkYIndex));
            ...
            int cStartY = Math.max(this.minY - sizeExtension,
                    Pos.BlockCoord.getMinYInSectionIndex(this.world, this.chunkYIndex));
    ...
    /**
     * Advances the sweep forward until finding a block with a box-colliding
     * VoxelShape
     *
     * @return null if no VoxelShape is left in the area, otherwise the next
     *         VoxelShape
     */
    @Override
    public VoxelShape computeNext() {
        while (true) {
            ...
            // The iteration order within a chunk section is chosen so that it causes a
            // mostly linear array access in the storage.
            // In net.minecraft.world.chunk.PalettedContainer.toIndex x gets the 4 least
            // significant bits, z the 4 above, and y the 4 even higher ones.
            ...
            if (this.cX < this.cEndX) {
                ...
                this.cZ = this.cStartZ;
                this.cY++;
                // stop condition was already checked using this.cIterated at the start of the
                // method
            }

            // using < minX and > maxX instead of <= and >= in vanilla, because minX, maxX
            // are the coordinates
            // of the box that wasn't extended for oversized blocks yet.
            final int edgesHit = this.sectionOversizedBlocks ? (x < this.minX || x > this.maxX ? 1 : 0) +
            ...
            if (collisionShape != VoxelShapes.empty() && collisionShape != null /*
                                                                                 * collisionShape should never be null,
                                                                                 * but we received crash reports.
                                                                                 */) {
                VoxelShape collidedShape = getCollidedShape(this.box, this.shape, collisionShape, x, y, z);
                if (collidedShape != null) {
                    if (z >= this.maxHitZ
                            && (z > this.maxHitZ || y >= this.maxHitY && (y > this.maxHitY || x > this.maxHitX))) {
                        this.maxHitX = x;
                        this.maxHitY = y;
                        this.maxHitZ = z;
    ...
    /**
     * This is an artifact from vanilla which is used to avoid testing shapes in the
     * extended portion of a volume
     * unless they are a shape which exceeds their voxel. Pistons must be
     * special-cased here.
     *
    ...
    /**
     * Checks if the {@param entityShape} or {@param entityBox} intersects the given
     * {@param shape} which is translated
     * to the given position. This is a very specialized implementation which tries
     * to avoid going through VoxelShape
     * for full-cube shapes.
     *
     * @return A {@link VoxelShape} which contains the shape representing that which
     *         was collided with, otherwise null
     */
    private static VoxelShape getCollidedShape(Box entityBox, VoxelShape entityShape, VoxelShape shape, int x, int y,
            int z) {
    ...
    /**
     * Checks the cached information whether the {@param chunkY} section of the
     * {@param chunk} has oversized blocks.
     *
    ...
    private static boolean hasChunkSectionOversizedBlocks(Chunk chunk, int chunkY) {
        ...
        return true; // like vanilla, assume that a chunk section has oversized blocks, when the
                     // section mixin isn't loaded
    }
    ...
    public List<VoxelShape> collectAll() {
        ...
        if (collisions.size() >= 2) {
            ...
            // Part of a fix of wrong movement when last collision results in movement
            // smaller than 1e-7. Changing which collision is the last one will change the
            // result. https://github.com/CaffeineMC/lithium-fabric/issues/443
